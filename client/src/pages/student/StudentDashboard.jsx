                ...
                {jobs.slice(0, 4).map((job, idx) => (
                  ...
                  >
                    ...
                    {job.skillsRequired && job.skillsRequired.length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        {job.skillsRequired.slice(0, expandedJobs.has(job._id) ? undefined : 3).map((skill, jdx) => (
                          <AnimatedBadge
                            key={jdx}
                            text={skill}
                            variant="tag"
                            className="bg-gray-100 text-gray-700 border-gray-200" // Custom style for these if needed, or stick to variant
                          />
                        ))}
                        {!expandedJobs.has(job._id) && job.skillsRequired.length > 3 && (
                          <AnimatedBadge
                            text={`+${job.skillsRequired.length - 3}`}
                            variant="tag"
                            onClick={(e) => {
                              e.stopPropagation();
                              toggleJobSkills(job._id);
                            }}
