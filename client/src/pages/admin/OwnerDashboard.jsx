    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="flex pt-16 h-screen overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
          ...
          >
            {[
              { title: 'Total Students', value: stats?.totalStudents || 0, icon: Users, color: 'purple', sub: '+12% this month' },
              { title: 'Partner Colleges', value: stats?.totalColleges || 0, icon: School, color: 'pink', sub: 'Across 14 states' },
              { title: 'Open Opportunities', value: stats?.totalJobs || 0, icon: Briefcase, color: 'blue', sub: '24 new today' },
              { title: 'System Activity', value: 'High', icon: Activity, color: 'green', sub: '99.9% Up-time' }
            ].map((card, i) => (
              <motion.div key={i} variants={itemVariants}>
                <GlassCard glow className="p-6 border-white/50 group h-full">
                  ...
                  <p className={`text-xs font-bold mt-2 text-${card.color}-600/70`}>{card.sub}</p>
                </GlassCard>
              </motion.div>
            ))}
          </motion.div>
            ...
            >
              <GlassCard className="p-8 border-white/60 h-full" glow={false}>
                <div className="flex items-center justify-between mb-8">
                  ...
                  <select className="bg-slate-50 border border-slate-200 text-slate-600 text-xs font-bold py-2 px-4 rounded-xl focus:outline-none">
            ...
            <motion.div variants={itemVariants} initial="hidden" animate="visible">
              <GlassCard className="p-6 border-white/60 border-t-4 border-t-purple-500" glow={false}>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button className="text-[10px] font-bold uppercase text-purple-600 hover:underline">View All</button>
                </div>
            ...
            <motion.div variants={itemVariants} initial="hidden" animate="visible" transition={{ delay: 0.1 }}>
              <GlassCard className="p-6 border-white/60 border-t-4 border-t-pink-500" glow={false}>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button className="text-[10px] font-bold uppercase text-pink-600 hover:underline">View All</button>
                </div>
                <div className="space-y-4">
                  {recentStudents.length > 0 ? recentStudents.map((stu, idx) => (
                    <div key={idx} className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-100 to-pink-100 flex items-center justify-center font-black text-xs text-purple-600">
                        {stu.name.charAt(0)}
                      </div>
                      <div>
                        <p className="text-sm font-bold text-slate-800 leading-none">{stu.name}</p>
            ...
            <motion.div variants={itemVariants} initial="hidden" animate="visible" transition={{ delay: 0.2 }}>
              <GlassCard className="p-6 border-white/60 border-t-4 border-t-indigo-500" glow={false}>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button className="text-[10px] font-bold uppercase text-indigo-600 hover:underline">Monitor</button>
                </div>
                <div className="space-y-4">
                  {recentColleges.length > 0 ? recentColleges.map((col, idx) => (
                    <div key={idx} className="flex flex-col gap-1 p-3 rounded-xl bg-slate-50/50 hover:bg-white transition-all border border-transparent hover:border-slate-100">
                      <p className="text-sm font-bold text-slate-800 leading-none">{col.name}</p>
